# -*- coding: utf-8 -*-
"""
Created on Wed Aug 14 16:15:19 2019
...
#os.chdir(os.path.join(os.path.expanduser('~'),'/Users/apple/Desktop/Code/'))
...
def get_Prob(e, tm, nbInt):
    ...
    indicator = np.zeros((len(e), nbInt))
    for i in np.array(range(0, len(e))):
        if(e[i] == 1):
            tmp = np.zeros((nbInt, ))
            tmp[(interval[i].astype("int") - 1)] = 1
            indicator[i] = tmp
        else:
            tmp = np.ones((nbInt, ))
            tmp[:(interval[i].astype("int"))] = np.zeros(((interval[i].astype("int32")), ))
            indicator[i] = tmp
...
def CensoringProb(Y, T):
    ...
    G[1, G[1, :] == 0] = G[1, G[1, :] != 0][-1]  #fill 0 with smallest probability (to prevent nan values)
    
    return G


def weighted_c_index(Y_train, T_train, Prediction, Y_test, T_test, Time):
    ...
    Den = 0
    for i in range(N):
        tmp_idx = np.where(G[0,:] >= T_test[i])[0] ##index of subjects whose time is >= T_test[i]

        if len(tmp_idx) == 0:
            W = (1. / G[1, -1])**2 ##G(T_i-)
        else:
            W = (1. / G[1, tmp_idx[0]])**2 ##G(T_i)

        A[i, np.where(T_test[i] < T_test)] = 1. * W ## I(T_i<T_j) * W
        Q[i, np.where(Prediction[i] > Prediction)] = 1. ## P_i > P_j

        if (T_test[i]<=Time and Y_test[i]==1):
            N_t[i,:] = 1.

    Num  = np.sum(((A) * N_t) * Q) ##sum all the elements in the array
    Den  = np.sum((A) * N_t)
...
#    inpshape = xtr.shape[1]
    ...
    riskpred= Dense(units=int(units3),  activation='softmax', name='predicted_risk')(merge_layer)
    ...
    Ind_prob = get_Prob(E_tr, TM_tr, int(units3))
...
def chimpred_new(m_MLP, y_train, x_test, y_test, eval_t = None):
    e_train = y_train[:, 0]
    tm_train = y_train[:, 1]
    e_test = y_test[:, 0]
    tm_test = y_test[:, 1]
    
    Xdat_test = normz(x_test[:,:-numcovs])
    Cdat_test = normz(x_test[:,-numcovs:])
    
    preds = m_MLP.predict([Xdat_test, Cdat_test], batch_size=1)[1]
    nbInt = preds.shape[1]
    
    if eval_t is None:
        eval_t = [int(np.percentile(np.array([tm_train, tm_test]).reshape(-1), 25)), int(np.percentile(np.array([tm_train, tm_test]).reshape(-1), 50)), int(np.percentile(np.array([tm_train, tm_test]).reshape(-1), 75))]
    
    result = np.zeros(len(eval_t))
    
    for t, t_time in enumerate(eval_t):
        if t_time >= T_max:
            print('ERROR: evaluation horizon is out of range')
            result[t] = -1;
        else:
            tm_train = tm_train.reshape(-1)
            gap = T_max / (nbInt - 1)
            interval = np.ceil(t_time / gap)
            risk = np.sum(preds[:, :interval.astype("int")], axis = 1)
            result[t] = weighted_c_index(e_train, tm_train, risk, e_test, tm_test, t_time)
...
def hypersearch_DL(x_data, y_data, method, nfolds, nevals, batch_size, num_epochs, alpha_range, dro_range, units1_range, units2_range, units3_range, l1r_range):
    @optunity.cross_validated(x=x_data, y=y_data, num_folds=nfolds)
    def modelrun(x_train, y_train, x_test, y_test, alpha, dro, units1, units2, units3, l1r):
        mod_MLP = modfit(xtr=x_train, ytr=y_train, alpha = alpha, dro = dro, units1 = units1, units2 = units2, units3 = units3, l1r = l1r, lr = 10**(-4.83), batchsize = batch_size, numepochs = num_epochs)
        _, cv_C = chimpred_new(mod_MLP, y_train, x_test, y_test, eval_t = np.arange(int(T_max / units3), T_max, int(T_max / units3)))
        return cv_C
    optimal_pars, searchlog, _ = optunity.maximize(modelrun, num_evals=nevals, solver_name=method, alpha=alpha_range, l1r=l1r_range, units1 = units1_range, units2= units2_range, units3 = units3_range, dro=dro_range)
...
#opars, osummary = hypersearch_DL(x_data=xdat, y_data=ydat, method='particle swarm', nfolds=10, nevals=50, batch_size=16, num_epochs=100, units3_range=[5,100], l1r_range=[-7,-4], dro_range=[.1,.9], units1_range=[75,250], units2_range=[5,20], alpha_range=[0.3,0.7])
with open('my3dall_new_prob_fix_copy_output_units3=35.pkl', 'rb') as f: inputdata_list=pickle.load(f)
opars = inputdata_list[2]
del inputdata_list

#(1b) using optimal hyperparameters, train a model on full sample
omMLP = modfit(xtr = xdat, ytr = ydat, alpha = opars['alpha'], dro = opars['dro'], units1 = opars['units1'], units2 = opars['units2'], units3 = opars['units3'], lr=10**(-4.83), l1r = opars['l1r'], batchsize=16, numepochs=100)

#(1c) Compute Harrell's Concordance index
predfull, C_app = chimpred_new(omMLP, ydat, xdat, ydat, eval_t = np.arange(100, 4400, 100))
...
#    b_inds = dlinbag[b]
    ...
    bpars = opars
    bmMLP = modfit(xtr=xboot, ytr=yboot, alpha = opars['alpha'], dro = opars['dro'], units1 = opars['units1'], units2 = opars['units2'], units3 = opars['units3'], lr=10**(-4.83), l1r = opars['l1r'], batchsize=16, numepochs=100)
    ...
    predboot, Cb_boot     = chimpred_new(bmMLP, yboot, xboot, yboot, eval_t = np.arange(100, 4400, 100))
    ...
    predbootfull, Cb_full = chimpred_new(bmMLP, yboot, xdat, ydat, eval_t = np.arange(100, 4400, 100))
...
with open('my3dall_new_prob_fix_output_unit3=35.pkl', 'wb') as f: pickle.dump([idat, ydat, preds_full, preds_boot, preds_bootfull, inds_inbag, C_app, Cb_opts, opars],f)

#with open('P:/myWS/my3dall_new_prob_fix_output_unit3=35.pkl', 'rb') as f: idat, ydat, preds_full, preds_boot, preds_bootfull, inds_inbag, C_app, Cb_opts, opars = pickle.load(f)
